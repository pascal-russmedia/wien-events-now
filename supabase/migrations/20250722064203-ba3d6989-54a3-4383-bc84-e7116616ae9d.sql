...
DROP FUNCTION public.get_expanded_future_events(text,text,text,date,date,date,integer,integer);
DROP FUNCTION public.get_home_page_events(text);

-- Recreate get_expanded_future_events function to include city
CREATE OR REPLACE FUNCTION public.get_expanded_future_events(region_filter text DEFAULT 'Vorarlberg'::text, category_filter text DEFAULT NULL::text, subcategory_filter text DEFAULT NULL::text, start_date_filter date DEFAULT NULL::date, end_date_filter date DEFAULT NULL::date, single_date_filter date DEFAULT NULL::date, limit_count integer DEFAULT 100, offset_count integer DEFAULT 0)
 RETURNS TABLE(id text, name text, category text, subcategory text, description text, region text, subregion text, city text, host text, address text, state text, popularity_score integer, trust_score integer, event_date date, start_time text, end_time text, image text, price_type text, price_amount numeric, link text, featured boolean, added_by text, added_by_email text, created_at timestamp with time zone, updated_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
AS $function$DECLARE
...
-- Recreate get_home_page_events function to include city
CREATE OR REPLACE FUNCTION public.get_home_page_events(region_filter text DEFAULT 'Vorarlberg'::text)
 RETURNS TABLE(id text, name text, category text, subcategory text, description text, region text, subregion text, city text, host text, address text, state text, popularity_score integer, trust_score integer, event_date date, start_time text, end_time text, image text, price_type text, price_amount numeric, link text, featured boolean, added_by text, created_at timestamp with time zone, updated_at timestamp with time zone)
 LANGUAGE plpgsql
AS $function$
DECLARE
  today_date date := CURRENT_DATE;
